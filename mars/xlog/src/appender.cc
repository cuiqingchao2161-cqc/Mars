...
std::string FormatBinaryLogRecord(const char* _tag, const void* _data, size_t _len) {
    std::string record;
    record.reserve(64 + (_tag ? strlen(_tag) : 0) + _len);
    record.append("[QYLOG_BINARY type=binary tag=");
    record.append(_tag ? _tag : "");
    record.append(" length=");
    record.append(std::to_string(_len));
    record.append("]\n");
    if (_data && _len > 0) {
        record.append(static_cast<const char*>(_data), _len);
    }
    record.append("\n[/QYLOG_BINARY]\n");
    return record;
}

void XloggerAppender::WriteBytes(const XLoggerInfo* _info, const void* _data, size_t _len) {
    if (log_close_ || nullptr == _data || 0 == _len)
        return;

    SCOPE_ERRNO();

    std::string record = FormatBinaryLogRecord(_info ? _info->tag : "", _data, _len);
    if (kAppenderSync == config_.mode_)
        __WriteRawSync(record.data(), record.size());
    else
        __WriteRawAsync(record.data(), record.size(), _info);
}
...
void XloggerAppender::__WriteRawSync(const void* _data, size_t _len) {
    AutoBuffer tmp_buff;
    if (!log_buff_->Write(_data, _len, tmp_buff))
        return;

    __Log2File(tmp_buff.Ptr(), tmp_buff.Length(), false);
}

void XloggerAppender::__WriteRawAsync(const void* _data, size_t _len, const XLoggerInfo* _info) {
    ScopedLock lock(mutex_buffer_async_);
    if (nullptr == log_buff_)
        return;

    if (!log_buff_->Write(_data, _len))
        return;
