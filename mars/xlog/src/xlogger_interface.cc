...
static std::map<std::string, std::string>& GetGlobalPrefixKeyMap() {
    static std::map<std::string, std::string> sg_map;
    return sg_map;
}

std::string MakeXloggerInstanceKey(const XLogConfig& _config) {
    return _config.cachedir_ + "\n" + _config.logdir_ + "\n" + _config.nameprefix_;
}
...
XloggerCategory* NewXloggerInstance(const XLogConfig& _config, TLogLevel _level) {
    ...
    const std::string instance_key = MakeXloggerInstanceKey(_config);
    auto it = GetGlobalInstanceMap().find(instance_key);
    ...
    GetGlobalInstanceMap()[instance_key] = category;
    GetGlobalPrefixKeyMap()[_config.nameprefix_] = instance_key;
    return category;
}
...
mars::comm::XloggerCategory* GetXloggerInstance(const char* _nameprefix) {
    ...
    auto key_it = GetGlobalPrefixKeyMap().find(_nameprefix);
    if (key_it != GetGlobalPrefixKeyMap().end()) {
        it = GetGlobalInstanceMap().find(key_it->second);
        if (it != GetGlobalInstanceMap().end()) {
            return it->second;
        }
    }
...
void ReleaseXloggerInstance(const char* _nameprefix) {
    ...
    if (it == GetGlobalInstanceMap().end()) {
        ...
        it = GetGlobalInstanceMap().find(key_it->second);
        if (it == GetGlobalInstanceMap().end()) {
            GetGlobalPrefixKeyMap().erase(key_it);
            return;
        }
        GetGlobalPrefixKeyMap().erase(key_it);
    }
...
void XloggerWriteBytes(uintptr_t _instance_ptr, const XLoggerInfo* _info, const void* _data, size_t _len) {
    if (0 == _instance_ptr || nullptr == _data || 0 == _len) {
        return;
    }

    XloggerCategory* category = reinterpret_cast<XloggerCategory*>(_instance_ptr);
    XloggerAppender* appender = reinterpret_cast<XloggerAppender*>(category->GetAppender());
    appender->WriteBytes(_info, _data, _len);
}
