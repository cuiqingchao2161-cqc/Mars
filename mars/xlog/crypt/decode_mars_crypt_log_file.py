...
def get_zstd():
    try:
        ...
        raise RuntimeError("zstandard module is required to decode zstd-compressed xlog files") from exc
    return zstd
...
def get_pyelliptic():
    try:
        ...
        raise RuntimeError("pyelliptic module is required to decode encrypted xlog files") from exc
    return pyelliptic
...
def IsGoodLogBuffer(_buffer, _offset, count):
    ...
    length = unpack_from("I", _buffer, _offset + headerLen - 4 - crypt_key_len)
...
def DecodeBuffer(_buffer, _offset, _outbuffer):
    ...
    if not ret[0]:
        ...
        if -1==fixpos: 
            ...
            append_text(_outbuffer, "[F]decode_log_file.py decode error len=%d, result:%s \n" % (fixpos, ret[1]))
            _offset += fixpos 
    ...
    if MAGIC_NO_COMPRESS_START==magic_start or MAGIC_COMPRESS_START==magic_start or MAGIC_COMPRESS_START1==magic_start:
        ...
        append_text(_outbuffer, 'in DecodeBuffer _buffer[%d]:%d != MAGIC_NUM_START' % (_offset, magic_start))
        return -1
    ...
    seq = unpack_from("H", _buffer, _offset + headerLen - 4 - crypt_key_len - 2 - 2)
    begin_hour = unpack_from("c", _buffer, _offset + headerLen - 4 - crypt_key_len - 1 - 1)
    end_hour = unpack_from("c", _buffer, _offset + headerLen - 4 - crypt_key_len - 1)

    global lastseq
    if seq != 0 and seq != 1 and lastseq != 0 and seq != (lastseq+1):
        append_text(_outbuffer, "[F]decode_log_file.py log seq:%d-%d is missing\n" % (lastseq + 1, seq - 1))
    ...
    try:
        ...
        if MAGIC_NO_COMPRESS_START1==_buffer[_offset] or MAGIC_SYNC_ZSTD_START==_buffer[_offset]:
            ...
            pyelliptic = get_pyelliptic()
            svr = pyelliptic.ECC(curve='secp256k1')
            client = pyelliptic.ECC(curve='secp256k1')
            client.pubkey_x = as_bytes(_buffer[_offset + headerLen - crypt_key_len:_offset + headerLen - crypt_key_len + crypt_key_len // 2])
            client.pubkey_y = as_bytes(_buffer[_offset + headerLen - crypt_key_len // 2:_offset + headerLen])
            ...
            if MAGIC_COMPRESS_START2==_buffer[_offset]:
                ...
                tmpbuffer = decompressor.decompress(as_bytes(tmpbuffer))
            else:
                zstd = get_zstd()
                decompressor = zstd.ZstdDecompressor()
                tmpbuffer = next(decompressor.read_from(ZstdDecompressReader(as_bytes(tmpbuffer)), 100000, 1000000))
        ...
        traceback.print_exc()  
        append_text(_outbuffer, "[F]decode_log_file.py decompress err, " + str(e) + "\n")
